package wylaga.view

import wylaga.view.display.displayables.Displayable
import wylaga.view.display.image.Base64Encoding

class ImageLoader(decode: (Base64Encoding) -> Displayable) {
    val enemy = decode(Base64Encoding("data:image/png;base64,",""))

    val redPlayerProjectile = decode(Base64Encoding("data:image/png;base64,","iVBORw0KGgoAAAANSUhEUgAAAAQAAAAPCAYAAADDNm69AAAAH0lEQVQoU2P8P2HCfwYkwAgWKCiACE2YwDD8BJB8CwAfTUjSYtSVFQAAAABJRU5ErkJgggAA"))
}