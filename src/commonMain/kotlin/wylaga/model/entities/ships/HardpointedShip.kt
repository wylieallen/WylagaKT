...
class HardpointedShip(x: Double, y: Double, width: Double, height: Double, velocity: Double, orientation: Entity.Orientation,
                      maxHealth: Double, points: Int, onDeath: (Ship) -> Unit, spawnProjectile: (Projectile, Any) -> Unit,
                      pilot: Pilot, private val hardpoint: Hardpoint)
    : Ship(x, y, width, height, velocity, orientation, maxHealth, points, onDeath, {ship -> hardpoint.weapon.fire(ship, hardpoint.x, hardpoint.y).forEach{spawnProjectile(it, hardpoint.weapon)}}, pilot) {
